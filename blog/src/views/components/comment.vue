  ...
  <div class="comment">
    <div class="commentBox">
      ...
      <input
        v-if="!token"
        type="email"
        v-model="queryData.user.email"
        ref="input_mail"
        @change="checkMail"
        name="user_email"
        placeholder="邮箱(用于通知回复信息):"
      >
    ...
    <div class="commentBox">
      ...
      <div v-else>
        <div class="comment" v-for="(item,index) in list" :key="item.id" v-bind:index="index">
          ...
          <div class="commemt-text" @click="changeCommenter(item,item)" v-html="item.content"></div>
          <div v-if="item.reply.length > 0">
            <div class="reply" v-for="(reply,num) in item.reply" :key="reply.id">
              <b>
                #{{index+1}}.{{num+1}}&nbsp;&nbsp;{{reply.user_name}}
                <span>回复</span>
                &nbsp;&nbsp;{{reply.target.user_name}}
                <span>{{reply.create_time}}</span>
              </b>
              <div class="commemt-text" @click="changeCommenter(item,reply)">{{reply.content}}</div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    checkMail: function() {
      ...
      const email = this.$refs.input_mail.value
      if (email) {
